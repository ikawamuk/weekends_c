...
t_world	set_world(void)
{
	...
	world.back_ground = construct_color(0.1, 0.1, 0.1);
	world.objects = construct_htl();
	// 柱
	add_htl(&world.objects, gen_cylinder(construct_vec(4, 0, -4), normalize(construct_vec(0, 1, 0)), 0.3, 1, gen_lambertian(construct_color(0.9, 0.9, 0.9))));
	...
	// ライト
	add_htl(&world.objects, gen_sphere(construct_vec(2, -1, -5), 0.5, gen_light(construct_vec(35, 35, 35)))); // lightの色tの強さは大きさ、距離などの複合で決めるらしい。むずかしい(--;)。
	return (world);
}
