...
t_world	set_world(void)
{
	...
	// 天井
	add_htl(&world.objects, gen_plane(construct_vec(0, 5, 0), normalize(construct_vec(0, -1, 0)), gen_lambertian(construct_color(0.7, 0.7, 0.7))));
	// もの
	...
	// ライト
	add_htl(&world.objects, gen_sphere(construct_vec(0, 5, -3), 0.5, gen_light(construct_vec(100, 100, 100)))); // lightの色tの強さは大きさ、距離などの複合で決めるらしい。むずかしい(--;)。
	return (world);
}
