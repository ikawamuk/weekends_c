...
t_camera	construct_camera(void)
{
	...
	camera.higher_left_corner = add_vec(sub_vec(camera.origin, div_vec(camera.horizontal, 2)), sub_vec(div_vec(camera.vertical, 2), construct_vec(0, 0, focal_length)));
...
t_ray	get_ray(t_camera camera, double u, double v)
{
	...
	ray = construct_ray(camera.origin, 
		sub_vec(add_vec(camera.higher_left_corner, mul_vec(camera.horizontal, u)), sub_vec(mul_vec(camera.vertical, v), camera.origin)));
