...
```
ここでランバート反射の $pScatter(ω_o, ω_i)$ を考える。前述したように物体表面での法線と散乱レイの角度を $θ$ とするとランバート面での光度(ここでは $pScatter$ )は $cosθ$ に比例する。つまり $pScatter(ω_o, ω_i) = C\cdot cosθ$ と表せる。またここでPDFの積分は1になることを思い出す。 $cosθ<0$ つまり散乱レイが表面より内側に散乱することはないので $pScatter(π/2<0\leq π) = 0$ とする。よって半球に関して $pScatter$ の積分し
```math
\begin{aligned}
